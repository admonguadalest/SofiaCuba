...
@UiController("test1_NotificacionesAumentos")
@UiDescriptor("notificaciones_aumentos.xml")
public class NotificacionesAumentos extends Screen {
    ...
    @Inject
    private TwinColumn<Concepto> twcConceptos;
    @Inject
    ...
    public void onBtnPrevisualizarClick() {
        ...
        List<Concepto> conceptos = new ArrayList(twcConceptos.getValue());
        if (conceptos.size()==0){
            notifications.create().withCaption("Seleccionar al menos un concepto").show();
            return;
        }
        try {
...
//                ht_objetos.put("diradmin", c.getDepartamento().getPropietarioEfectivo().getPersona().getDireccionDesdeNombre(Direccion.NOMBRE_DIRECCION_PROPIETARIO_CONTRATO_N19));
                    ...
                    ht_objetos.put("helper", new HelperNotificacionesAumentos(ccrraa));
